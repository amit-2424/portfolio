...
const Navbar = ({isDarkMode, setIsDarkMode}) => {
  ...
  return (
    <>
      ...
      <nav className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex align-center justify-between z-50 ${isScroll ? "bg-white bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20 " : ""}`}>
        <a href="#top">
            <Image src={ isDarkMode ? assets.logo_dark : assets.logo} alt='#' className='w-28 cursor-pointer mr-14' />
        </a>
        <ul className={`hidden md:flex items-center gap-4 lg:gap-8 rounded-full px-4 lg:px-12 py-3 bg-white shadow-sm bg-opacity-50 md:text-xs lg:text-sm ${isScroll? "" : "bg-white shadow-sm bg-opacity-50 dark:border dark:border-white/50 dark:bg-transparent"}`}>
